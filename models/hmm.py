...
class HMM(object):
    def __init__(self, N, M):
        """
        :param int N: 状态数，这里对应存在的标注的种类
        :param int M: 观测数，这里对应有多少不同的字
        """
        self.N = N
        self.M = M
    ...
    def train(self, word_lists, tag_lists, word2id, tag2id):
        """HMM 模型训练, 即根据训练语料对模型参数进行估计, 因为我们有观测序列以及其对应的状态序列, 
            所以我们可以使用极大似然估计的方法来估计隐马尔可夫模型的参数。

        :param list word_lists: 其中每个元素由字组成的列表, 如 ['担','任','科','员']
        :param list tag_lists: 其中每个元素是由对应的标注组成的列表, 如 ['O','O','B-TITLE', 'E-TITLE']
        :param dist word2id: 将字映射为ID
        :param dict tag2id: 将标注映射为ID
        """
    ...
    def decoding(self, word_list, word2id, tag2id):
        """使用Viterbi(维特比)算法对给定观测序列求状态序列, 这里就是对字组成的序列, 求其对应的标注。
            维特比算法实际是用动态规划解隐马尔可夫模型预测问题,
            即用动态规划求概率最大路径（最优路径）这时一条路径对应着一个状态序列

        :param list word_list: 由字组成的列表, 如 ['担','任','科','员']
        :param dist word2id: 将字映射为ID
        :param dict tag2id: 将标注映射为ID
        """
        ...
        # 初始化维比特矩阵 viterbi 它的维度为[状态数, 序列长度]
        # 其中 viterbi[i, j] 表示标注序列的第j个标注为i的所有单个序列(i_1, i_2, ..i_j)出现的概率最大值
        seq_len = len(word_list)
        viterbi = torch.zeros(self.N, seq_len)
        # backpointer 是跟 viterbi 一样大小的矩阵
        ...
        # Bt[word_id] 表示字为 word_id 的时候，对应各个标记的概率
